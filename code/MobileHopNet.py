...
class MobileHopeNetV2(nn.Module):
    def __init__(self, num_bins, width_mult=1.0):
        super(MobileHopeNetV2, self).__init__()
    ...
    def forward(self, x):
        ...
        pre_yaw = self.fc_yaw(x)  # (1, 66)

        pre_pitch = self.fc_pitch(x)  # (1, 66)

        pre_roll = self.fc_roll(x)  # (1, 66)
...
if __name__ == '__main__':
    ...
    model = MobileHopeNetV2(num_bins=66)
    model.cuda(0)
